...
bitboard VERTICAL_OFFSETS_BITBOARDS[17]
    = { 0ull, 255ull, 65535ull, 16777215ull, 4294967295ull, 1099511627775ull, 281474976710655ull,
          72057594037927935ull, 18446744073709551615ull, 18446744073709551360ull,
          18446744073709486080ull, 18446744073692774400ull, 18446744069414584320ull,
          18446742974197923840ull, 18446462598732840960ull, 18374686479671623680ull, 0ull };

bitboard HORIZONTAL_OFFSETS_BITBOARDS[17] = { 0ull, 72340172838076673ull, 217020518514230019ull,
    506381209866536711ull, 1085102592571150095ull, 2242545357980376863ull, 4557430888798830399ull,
    9187201950435737471ull, 18446744073709551615ull, 18374403900871474942ull,
    18229723555195321596ull, 17940362863843014904ull, 17361641481138401520ull,
    16204198715729174752ull, 13889313184910721216ull, 9259542123273814144ull, 0ull };

bitboard offset_to_bitboard[64] = { 0x8000000000000000, 0x4000000000000000, 0x2000000000000000,
    ...
    0x1000000, 0x800000, 0x400000, 0x200000, 0x100000, 0x80000, 0x40000, 0x20000, 0x10000, 0x8000,
    0x4000, 0x2000, 0x1000, 0x800, 0x400, 0x200, 0x100, 0x80, 0x40, 0x20, 0x10, 0x8, 0x4, 0x2,
    0x1 };
...
chess *chess_create() {
    ...
    game->kings &= (~offset_to_bitboard[0] & (~offset_to_bitboard[7]) & (~offset_to_bitboard[56])
...
void print_game_state(chess *game) {
    ...
    for (int i = 56; i != -8; i++) {
        ...
        switch (focus_piece) {
        case none: printf("%c", ' '); break;
        case rook: printf("%c", 'R' + upset); break;
        case knight: printf("%c", 'N' + upset); break;
        case bishop: printf("%c", 'B' + upset); break;
        case queen: printf("%c", 'Q' + upset); break;
        case king: printf("%c", 'K' + upset); break;
        case pawn: printf("%c", 'P' + upset); break;
...
move encode_string(char *focus_move) {
    if (focus_move[0] == 'i' && focus_move[1] == 'n' && focus_move[2] == 'i' && focus_move[3] == 't'
        && focus_move[4] == '\0') {
...
void flip_perspective(chess *game) {
    game->pawns = (((((game->pawns ^ 1) & (~30)) | (((game->pawns ^ 1) & 6) << 2)
                        | (((game->pawns ^ 1) & 24) >> 2))
                       & (~(ROW_1 | compass(ROW_1, 0, 7))))
                   | (compass((((game->pawns ^ 1) & (~30)) | (((game->pawns ^ 1) & 6) << 2)
                                  | (((game->pawns ^ 1) & 24) >> 2)),
                          0, -7)
                       & ROW_1)
                   | (compass((((game->pawns ^ 1) & (~30)) | (((game->pawns ^ 1) & 6) << 2)
                                  | (((game->pawns ^ 1) & 24) >> 2))
                                  & ROW_1,
                       0, 7))); //swaps turn, swaps castle rights, swaps info and en-passant rows
...
offset get_starting_offset(move focus_move) {
    return focus_move & 63;
}

offset get_ending_offset(move focus_move) {
    return (focus_move >> 6) & 63;
}
...
bool is_promotion(chess *game, move focus_move) {
    return (get_starting_piece(game, focus_move) == pawn) && (get_ending_offset(focus_move) > 55);
}
...
bool is_legal_bit_move(chess *game, move focus_move) {
...
void add_piece(chess *game, piece focus_piece, bool is_friendly, offset focus_offset) {
    ...
    switch (focus_piece) {
    case none: remove_piece(game, focus_offset);
    case pawn:
...
move random_bit_move(chess *game) {
    ...
    int temp = random_in_range(0, pop_count(start_board) - 1);
    for (int i = 0; i < temp; i++) {
        starting_offset = get_next_offset(start_board, starting_offset);
    }
    temp = random_in_range(0, pop_count(end_board) - 1);
    for (int i = 0; i < temp; i++) {
