...
int set_pseudo_legal_moves(chess *game, move *legal_moves) {
    ...
    legal_moves_count = add_single_pawn_pushes(game, legal_moves, legal_moves_count);
    legal_moves_count = add_double_pawn_pushes(game, legal_moves, legal_moves_count);
    legal_moves_count = add_right_pawn_captures(game, legal_moves, legal_moves_count);
    legal_moves_count = add_left_pawn_captures(game, legal_moves, legal_moves_count);

    legal_moves_count = add_en_passant_capture(game, legal_moves, legal_moves_count);
    ...
    legal_moves_count = add_orthogonal_moves(game, legal_moves, legal_moves_count);
...
int set_legal_moves(chess *game, move *legal_moves) {
    ...
    offset friendly_king_offset = get_next_offset(true_kings(game) & game->friendly_pieces, -1);
...
bool is_legal_move(chess *game, move focus_move) {
    ...
    offset enemy_king_offset = get_next_offset(true_kings(game) & game->enemy_pieces, -1);
    bool is_legal = true;
    if (attacking_offset(game, enemy_king_offset)) {
        is_legal = false;
    }
    bitboard castle_pass_through_board = get_castle_pass_through_board(game, focus_move);
    for (offset pass = -1; (pass = get_next_offset(castle_pass_through_board, pass)) < 64;) {
...
int add_en_passant_capture(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    if (compass(offset_to_bitboard[en_passant], 1, 4) & game->pawns & game->friendly_pieces) {
        legal_moves[legal_moves_count] = encode_move(33 + en_passant, 40 + en_passant, none);
        legal_moves_count++;
        legal_moves[legal_moves_count] = NULL_MOVE;
    }
    if (compass(offset_to_bitboard[en_passant], -1, 4) & game->pawns & game->friendly_pieces) {
        legal_moves[legal_moves_count] = encode_move(31 + en_passant, 40 + en_passant, none);
        legal_moves_count++;
        legal_moves[legal_moves_count] = NULL_MOVE;
    }
    return legal_moves_count;
...
int add_castling_moves(chess *game, move *legal_moves, int legal_moves_count) {
    if ((game->kings & offset_to_bitboard[0])
        && !((game->friendly_pieces | game->enemy_pieces)
             & (offset_to_bitboard[1] | offset_to_bitboard[2] | offset_to_bitboard[3]))) {
...
int add_double_pawn_pushes(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    output_squares = (~entire_board) & compass(output_squares, 0, 1);
    for (offset end_index = -1; (end_index = get_next_offset(output_squares, end_index)) < 64;) {
        legal_moves[legal_moves_count] = encode_move(end_index - 16, end_index, none);
        legal_moves_count++;
        legal_moves[legal_moves_count] = NULL_MOVE;
    }
    return legal_moves_count;
...
int add_single_pawn_pushes(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    output_squares = (~entire_board) & compass(output_squares, 0, 1);
    for (offset end_index = -1; (end_index = get_next_offset(output_squares, end_index)) < 64;) {
        legal_moves[legal_moves_count] = encode_move(end_index - 8, end_index, none);
        legal_moves_count++;
        if (end_index > 55) {
            legal_moves[legal_moves_count] = encode_move(end_index - 8, end_index, rook);
            legal_moves_count++;
            legal_moves[legal_moves_count] = encode_move(end_index - 8, end_index, bishop);
            legal_moves_count++;
            legal_moves[legal_moves_count] = encode_move(end_index - 8, end_index, knight);
            legal_moves_count++;
        }
        legal_moves[legal_moves_count] = NULL_MOVE;
    }
...
int add_left_pawn_captures(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    for (offset end_index = -1; (end_index = get_next_offset(output_squares, end_index)) < 64;) {
        legal_moves[legal_moves_count] = encode_move(end_index - 7, end_index, none);
        legal_moves_count++;
        if (end_index > 55) {
            legal_moves[legal_moves_count] = encode_move(end_index - 7, end_index, rook);
            legal_moves_count++;
            legal_moves[legal_moves_count] = encode_move(end_index - 7, end_index, bishop);
            legal_moves_count++;
            legal_moves[legal_moves_count] = encode_move(end_index - 7, end_index, knight);
            legal_moves_count++;
        }
        legal_moves[legal_moves_count] = NULL_MOVE;
    }
...
int add_right_pawn_captures(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    output_squares = (game->enemy_pieces) & compass(output_squares, 1, 1);
    for (offset end_index = -1; (end_index = get_next_offset(output_squares, end_index)) < 64;) {
        legal_moves[legal_moves_count] = encode_move(end_index - 9, end_index, none);
        legal_moves_count++;
        if (end_index > 55) {
            legal_moves[legal_moves_count] = encode_move(end_index - 9, end_index, rook);
            legal_moves_count++;
            legal_moves[legal_moves_count] = encode_move(end_index - 9, end_index, bishop);
            legal_moves_count++;
...
int add_king_moves(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    output_squares &= (~game->friendly_pieces);
    for (offset end_index = -1; (end_index = get_next_offset(output_squares, end_index)) < 64;) {
        legal_moves[legal_moves_count] = encode_move(get_next_offset(king, -1), end_index, none);
        legal_moves_count++;
        legal_moves[legal_moves_count] = NULL_MOVE;
    }
    return legal_moves_count;
...
int add_diagonal_moves(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    for (offset start_index = -1; (start_index = get_next_offset(diagonals, start_index)) < 64;) {
        bitboard output_squares = get_diagonal_destination_bitboard(game, start_index);
        for (offset end_index = -1;
             (end_index = get_next_offset(output_squares, end_index)) < 64;) {
            legal_moves[legal_moves_count] = encode_move(start_index, end_index, none);
            legal_moves_count++;
            legal_moves[legal_moves_count] = NULL_MOVE;
        }
    }
...
int add_orthogonal_moves(chess *game, move *legal_moves, int legal_moves_count) {
    ...
    for (offset start_index = -1; (start_index = get_next_offset(orthogonals, start_index)) < 64;) {
        bitboard output_squares = get_orthogonal_destination_bitboard(game, start_index);
        for (offset end_index = -1;
             (end_index = get_next_offset(output_squares, end_index)) < 64;) {
            legal_moves[legal_moves_count] = encode_move(start_index, end_index, none);
            legal_moves_count++;
...
int add_knight_moves(chess *game, move *legal_moves, int legal_moves_count) {
    bitboard knights = game->friendly_pieces & (~true_pawns(game)) & (~game->orthogonal_pieces)
                       & ~(game->diagonal_pieces) & (~true_kings(game));
    for (offset start_index = -1; (start_index = get_next_offset(knights, start_index)) < 64;) {
        bitboard output_squares = get_knight_destination_bitboard(game, start_index);
        for (offset end_index = -1;
             (end_index = get_next_offset(output_squares, end_index)) < 64;) {
            legal_moves[legal_moves_count] = encode_move(start_index, end_index, none);
            legal_moves_count++;
...
bool attacked_offset(chess *game, offset focus_offset) {
    ...
    bitboard knights = (game->friendly_pieces | game->enemy_pieces) & (~game->orthogonal_pieces)
                       & (~game->diagonal_pieces) & ~(true_pawns(game)) & ~(true_kings(game));
    bitboard knight_attackers
        = game->enemy_pieces & knights & get_knight_destination_bitboard(game, focus_offset);
    bitboard diagonal_attackers = game->enemy_pieces & game->diagonal_pieces
                                  & get_diagonal_destination_bitboard(game, focus_offset);
    bitboard orthogonal_attackers = game->enemy_pieces & game->orthogonal_pieces
                                    & get_orthogonal_destination_bitboard(game, focus_offset);
    bitboard king_attackers
        = game->enemy_pieces & true_kings(game) & get_king_destination_bitboard(game, focus_offset);
    bitboard pawn_attackers = game->enemy_pieces & game->pawns
                              & (compass(attack_board, -1, 1) | compass(attack_board, 1, 1));
    if (pawn_attackers | knight_attackers | diagonal_attackers | orthogonal_attackers
        | king_attackers) {
        return true;
    }
    return false;
...
bool attacking_offset(chess *game, offset focus_offset) {
    ...
    game->friendly_pieces = game->enemy_pieces;
    game->enemy_pieces = temp;
    bitboard pawn_attackers = game->friendly_pieces & game->pawns
                              & (compass(attack_board, -1, -1) | compass(attack_board, 1, -1));
    if (pawn_attackers | knight_attackers | diagonal_attackers | orthogonal_attackers
        | king_attackers) {
        return true;
...
bitboard get_knight_destination_bitboard(chess *game, offset start_index) {
    ...
    return (~game->friendly_pieces)
           & (compass(start_board, 2, 1) | compass(start_board, 1, 2) | compass(start_board, 2, -1)
               | compass(start_board, -1, 2) | compass(start_board, -2, 1)
               | compass(start_board, 1, -2) | compass(start_board, -2, -1)
               | compass(start_board, -1, -2));
}
