...
full_chess *full_chess_create() {
    ...
    game->board_history[0].kings = 0x8900000000000089; // contains castle rights in corners
    game->board_history[0].pawns &= ~((0x0) & offset_to_bitboard[63]); // board isn't color-flipped
    game->board_history[0].pawns
        &= ~((0x0) & offset_to_bitboard[62]); // player to move isn't in check
    game->board_history[0].pawns &= ~((0x0) & offset_to_bitboard[61]); // game isn't over
    game->board_history[0].pawns
...
void set_from_fen(full_chess *game, char *fen) {
    ...
    while (focus_char != ' ') {
        ...
        if (is_numeric) {
            ...
            case 'K':
                focus_piece = king;
                add_to_board(game->current_board, focus_piece, board_index, is_friendly);
                board_index++;
                if (board_index % 8 == 0) {
                    board_index -= 16;
                }
    ...
    if (focus_char != '-') {
        while (focus_char != ' ') {
            switch (focus_char) {
            case 'K': game->current_board->kings |= offset_to_bitboard[7]; break;
            case 'Q': game->current_board->kings |= offset_to_bitboard[0]; break;
            case 'k': game->current_board->kings |= offset_to_bitboard[63]; break;
            case 'q': game->current_board->kings |= offset_to_bitboard[56]; break;
            default: break;
            }
            str_index++;
            focus_char = fen[str_index];
...
char *get_fen(full_chess *game, char *fen) {
    ...
    offset en_passant_offset = get_next_offset(game->current_board->pawns & ROW_1, -1);
    if (en_passant) {
    ...
    int full_move_count = 1 + ((game->move_count - (game->current_board->pawns & 1)) / 2);
    if (full_move_count < 10) {
        ...
        str_index++;
        fen[str_index] = '0' + ((full_move_count - 100 * (full_move_count / 100)) / 10);
        str_index++;
        fen[str_index] = '0' + (full_move_count % 10);
        str_index++;
    }
...
void print_fen(full_chess *game) {
    ...
    if (full_move_count < 10) {
        ...
        printf("%c", '0' + ((full_move_count - 100 * (full_move_count / 100)) / 10));
        printf("%c", '0' + (full_move_count % 10));
    }
}
...
char get_piece_char_at_offset(full_chess *game, offset index) {
    ...
    bool is_friendly = offset_to_bitboard[index] & game->current_board->friendly_pieces;
    switch (focus_piece) {
    case none: return ' ';
    case queen: return is_friendly ? 'Q' : 'q';
    case rook: return is_friendly ? 'R' : 'r';
    case bishop: return is_friendly ? 'B' : 'b';
    case knight: return is_friendly ? 'N' : 'n';
    case pawn: return is_friendly ? 'P' : 'p';
    case king: return is_friendly ? 'K' : 'k';
    default: return '?';
    }
}
...
void print_move(full_chess *game, move focus_move) {
    ...
    piece starting_piece = get_piece_at_offset(game->current_board, starting_offset);
